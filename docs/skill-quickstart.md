...
`skills/verify-worktree-imports`
...
From this repository root, replace the destination with your test project's path:

```console
python skills/verify-worktree-imports/scripts/install_skill.py /absolute/project
```

The installer refuses an existing destination. It copies files only; the Python used
to copy the Skill does not select or install the project's test environment. On Windows
use a destination such as `D:/my-project`.
