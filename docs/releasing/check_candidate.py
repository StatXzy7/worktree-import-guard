...
def main() -> int:
    ...
    wheel = dist / "worktree_import_guard-0.1.2-py3-none-any.whl"
    sdist = dist / "worktree_import_guard-0.1.2.tar.gz"
    ...
    try:
        ...
        with zipfile.ZipFile(wheel) as archive:
            metadata = BytesParser().parsebytes(
                archive.read("worktree_import_guard-0.1.2.dist-info/METADATA")
            )
            assert metadata["Name"] == "worktree-import-guard" and metadata["Version"] == "0.1.2"
        ...
        with tarfile.open(sdist) as archive:
            member = archive.extractfile("worktree_import_guard-0.1.2/PKG-INFO")
            assert member is not None
            assert BytesParser().parsebytes(member.read())["Version"] == "0.1.2"
        run(
